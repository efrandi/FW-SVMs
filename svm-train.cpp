...
void read_problem(const char *filename)
{
	...
	else if ( param.svm_type == MCVM )
	{
		...
		if (param.mcvm_type == ADSVM){
			...
			param.sameclassYDP = (float) num_classes - 1;
			param.diffclassYDP = (float) -1;
			...
			param.sameclassYDP = (float) 1;
			param.diffclassYDP = (float) (3*num_classes - 4)/(num_classes*(num_classes-1));
			printf("Using Asharaf codes: sameclassYDP: %.6g, diffclassYDP: %.6g  \n", param.sameclassYDP, param.diffclassYDP);
		}	
		else if (param.mcvm_type == ADNORM){
	
			param.sameclassYDP = (float)1;
			param.diffclassYDP = (float) -1/(num_classes-1);
			...
			param.diffclassYDP = (float)num_classes-4;
